...
class DesiredJointPublisher():
    ...
    def __init__(self, node, description):
        ...
        if use_gui:
            ...
            self.gui = DesiredJointPublisherGui("Joint State Publisher", self, num_rows)
            self.gui.show()
        else:
            self.gui = None
...
class DesiredJointPublisherGui(QWidget):
    sliderUpdateTrigger = Signal()

    def __init__(self, title, jsp, num_rows=0):
        super(DesiredJointPublisherGui, self).__init__()
        self.jsp = jsp
        self.joint_map = {}
